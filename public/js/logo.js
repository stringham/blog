var drawCurve = function(ctx,x,y,w,h,s,e) {
	...
	var pointAt = function(t) {
		//Tangent to an ellipse: (x(t),y(t))t = (w(cos伪 - tsin伪),h(sin伪 + tcos伪))
		//Note that if t=0, (x,y) = (w(cos伪),h(sin伪)), the point on the circle.
		//And if t=k, (x,y) = (w(cos伪 - ksin伪),h(sin伪 + kcos伪)), which is the control point we want.
		return {
			t:t,
...
Logo.prototype.animate = function(duration,synchronous){
	...
	function next(){
		...
		var p = Math.min(1,time/duration);
		me.drawCircle(p);
		if(!synchronous){
			me.drawR(p);
			me.drawS(p);
